...
import torchvision
from sound_synthesis2.utils.misc import instantiate_from_config, format_seconds
from sound_synthesis2.distributed.distributed import reduce_dict
from sound_synthesis2.distributed.distributed import is_primary, get_rank
from sound_synthesis2.utils.misc import get_model_parameters_info
from sound_synthesis2.engine.lr_scheduler import ReduceLROnPlateauWithWarmup, CosineAnnealingLRWithWarmup
from sound_synthesis2.engine.ema import EMA
...
class Solver(object):
    def __init__(self, config, args, model, dataloader, logger):
        ...
        if self.sample_iterations == 'epoch': # 改为2个epoch 采样一次
            self.sample_iterations = 2*self.dataloader['train_iterations'] # 4106
    ...
    def sample(self, batch, phase='train', step_type='iteration'):
        ...
        with torch.no_grad(): 
            ...
            for i in range(num_plots):
                ...
                torch.save(pre_content_codec, save_path + '/wav_pred_'+str(i) + '_epoch_' + str(self.last_epoch) + '_last_iter_' + str(self.last_iter) +'.pth')
        if self.ema is not None:
