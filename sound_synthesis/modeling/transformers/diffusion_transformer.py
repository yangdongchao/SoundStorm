...
def index_to_log_onehot(x, num_classes):  # 
    ...
    # print('x_onehot ', x_onehot.shape)
    log_x = torch.log(x_onehot.float().clamp(
        min=1e-30))  # 对one-hot 取log? [2, 2888, 1024]
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        if condition_emb_config is None:  # 不使用 conditional information
            ...
            self.condition_emb = instantiate_from_config(
                condition_emb_config)  # 加载能获得condition embedding的模型
        ...
        transformer_config['params'][
            'diffusion_step'] = diffusion_step  # 在transformer_conf文件中，加入这两个参数
        ...
        self.amp = False
        self.num_classes = self.transformer.content_emb.num_embed  # 直接调用 embeding里面的参数，已经被加1了
    ...
    def q_pred_one_timestep(self, log_x_t, t):  # q(xt|xt_1)
        ...
        log_probs[:, -1:, :] = log_add_exp(log_x_t[:, -1:, :] + log_1_min_ct,
                                           log_ct)
    ...
    def q_pred(self, log_x_start, t):  # q(xt|x0)
        ...
        t = (t + (self.num_timesteps + 1)) % (
            self.num_timesteps + 1)  # 保证在t在0到self.num_timesteps直接
        #根据时间步t,查找出需要的系数
        ...
        log_probs[:, :-1, :] = log_add_exp(
            log_x_start[:, :-1, :] + log_cumprod_at,
            log_cumprod_bt)  # M + log(e_(a-M)+e_(b-M))
        log_probs[:, -1:, :] = log_add_exp(
    ...
    def predict_start(self, log_x_t, cond_emb, t):  # p(x0|xt)
        ...
        zero_vector = torch.zeros(
            batch_size, 1,
            log_x_t.shape[2]).type_as(log_x_t) - 70  # ? (log(1e-30))?
    ...
    def q_posterior(self, log_x_start, log_x_t,
        ...
        ct_cumprod_vector = log_cumprod_ct.expand(
            -1, self.num_classes - 1, -1)  # log_x_start=log_x0_recon, b,1,1
        ...
        log_qt_one_timestep = self.q_pred_one_timestep(
            log_x_t, t)  # q(xt|xt_1), 只向前一步,因为用了 at,bt
    ...
    def p_pred(self, log_x, cond_emb,
               t):  # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
    ...
    def log_sample_categorical(
            self,
            logits):  # use gumbel to sample onehot vector from log probability
        uniform = torch.rand_like(logits)
        gumbel_noise = -torch.log(
            -torch.log(uniform + 1e-30) + 1e-30)  # 产生一定的噪声
        # print('logits ', logits)
        ...
        log_sample = index_to_log_onehot(
            sample, self.num_classes)  # 又把index转为log one-hot
        return log_sample

    def q_sample(self, log_x_start,
                 t):  # diffusion step, q(xt|x0) and sample xt
        log_EV_qxt_x0 = self.q_pred(log_x_start,
                                    t)  # 从x_0开始，往前走t步 (马尔科夫链),获得logq(xt|x0)
    ...
    def sample_time(self, b, device, method='uniform'):
        if method == 'importance':
            ...
            t = torch.multinomial(
                pt_all, num_samples=b, replacement=True)  # 采index 权重大的，采到的几率就越大
            ...
            t = torch.randint(
                ...
                device=device).long()  #从[0,num_timesteps]随机产生b个数
            # print('t ',t)
    ...
    def _train_loss(self, x, cond_emb, is_train=True):  # get the KL loss
        ...
        log_x_start = index_to_log_onehot(
            x_start,
            self.num_classes)  # 将数值代表，转换为由one-hot向量组成的矩阵,其中每个向量最大值所在的索引就是原始的值
        log_xt = self.q_sample(
            log_x_start=log_x_start, t=t)  # 通过采样获得 log_xt, 随机采得
        xt = log_onehot_to_index(log_xt)  # get b, 1024
        ############### go to p_theta function ###############
        log_x0_recon = self.predict_start(
            log_xt, cond_emb, t=t)  # P_theta(x0|xt)
        ...
        self.Lt_count.scatter_add_(
            dim=0, index=t, src=torch.ones_like(Lt2))  # 记录加的次数,也可理解为选择了时间步t的次数
    ...
    @property
    def device(self):
        return self.transformer.mul_head_logits[-1].Linear.weight.device
    ...
    def forward(self,
        ...
        if self.condition_emb is not None:  # self.condition_emb 表示提取文本特征的model
            with autocast(enabled=False):
                with torch.no_grad():
                    cond_emb = self.condition_emb(
                        input['condition_token'])  # from b,77 to b,77,512
                cond_emb = cond_emb.float()
        else:  # share condition embeding with content
            if input.get('condition_embed_token') is None:
                cond_emb = None
            else:
        ...
        if is_train is True:
            ...
            loss = loss.sum() / (sample_image.size()[0] *
                                 sample_image.size()[1])
    ...
    def sample(self,
        ...
        if input['condition_token'] is not None:
        ...
        if content_token is not None:
            sample_image = input['content_token'].type_as(
                input['content_token'])  # B,265

        if self.condition_emb is not None:  # do this
            with torch.no_grad():
                cond_emb = self.condition_emb(input[
                    'condition_token'])  # B x Ld x D   from b,77 to b,77,512
            cond_emb = cond_emb.float()
        else:  # share condition embeding with content
            if input.get('condition_embed_token', None) is not None:
                cond_emb = input['condition_embed_token'].float()
            else:
                cond_emb = None
        ...
        if start_step == 0:  # when filter_ratio==0
            ...
            zero_logits = torch.zeros(
                (batch_size, self.num_classes - 1, self.shape),
                device=device)  #b,256,265
            # print(self.shape)
            # assert 1==2
            one_logits = torch.ones(
                (batch_size, 1, self.shape), device=device)  #b,1,265
            mask_logits = torch.cat(
                (zero_logits, one_logits), dim=1)  # 每个token全是mask
            ...
            with torch.no_grad():
                for diffusion_index in range(start_step - 1, -1, -1):  # 99,0
                    ...
                    log_z = self.p_sample(log_z, cond_emb,
                                          t)  # log_z is log_onehot
        else:
            t = torch.full(
                (batch_size, ), start_step - 1, device=device, dtype=torch.long)
            log_x_start = index_to_log_onehot(sample_image,
                                              self.num_classes)  # 
        ...
        # print('content_token ', content_token.shape)
        # assert 1==2
        output = {
            'content_token': content_token
    ...
    def sample_uniform_only(self,
        ...
        if start_step == 0:  # when filter_ratio==0
            ...
            x_init = torch.randint(0, self.num_classes - 2,
    ...
    def sample_fast(self,
        ...
        if self.condition_emb is not None:
            with torch.no_grad():
                cond_emb = self.condition_emb(
                    input['condition_token'])  # B x Ld x D   #256*1024
            cond_emb = cond_emb.float()
        else:  # share condition embeding with content
            cond_emb = input['condition_embed_token'].float()
