...
from encodec.encodec.utils import _check_checksum
from encodec.encodec.utils import _get_checkpoint_url
from encodec.encodec.utils import _linear_overlap_add
...
class LMModel(nn.Module):
    ...
    def forward(self,
        ...
        logits = torch.stack(
            [self.linears[k](out) for k in range(K)], dim=1).permute(0, 3, 1, 2)
...
class EncodecModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = m.SEANetEncoder(
            channels=self.channels, norm=self.model_norm, causal=self.causal)
        self.decoder = m.SEANetDecoder(
            channels=self.channels, norm=self.model_norm, causal=self.causal)
        self.n_q = int(1000 * target_bandwidths[-1] //
