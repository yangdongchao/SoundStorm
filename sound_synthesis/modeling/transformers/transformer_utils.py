...
class Block(nn.Module):
    ...
    def forward(self, x, encoder_output, timestep, mask=None):
        if self.attn_type == "selfcross":
            a, att = self.attn1(
                self.ln1(x, timestep), encoder_output, mask=mask)
            x = x + a
            a, att = self.attn2(
                self.ln1_1(x, timestep), encoder_output, mask=mask)
            x = x + a
        elif self.attn_type == "selfcondition":
            a, att = self.attn(self.ln1(x, timestep), encoder_output, mask=mask)
            x = x + a
            x = x + self.mlp(self.ln2(
                x, encoder_output.long()))  # only one really use encoder_output
            return x, att
        else:  # 'self'
            a, att = self.attn(self.ln1(x, timestep), encoder_output, mask=mask)
            x = x + a
...
class Text2ImageTransformer(nn.Module):
    def __init__(
        ...
        #assert 1==2
        self.mul_head_logits = nn.ModuleList()
        for i in range(self.n_q):
            self.mul_head_logits.append(Classifer(n_embd, out_cls))
    ...
    def forward(self, input, cond_emb, t):
        ...
        # assert 1==2
        # emb = cont_emb.reshape(cont_emb.shape[0], self.n_q, -1, cont_emb.shape[-1])
        # emb = emb.mean(1) # 求 mean
        emb = cont_emb  # we make the mean pooling on in content_emb layer
        for block_idx in range(len(self.blocks)):
            if self.use_checkpoint is False:
                ...
                emb, att_weight = checkpoint(self.blocks[block_idx], emb,
                                             cond_emb, t.cuda())
        logits = []
        for layer in self.mul_head_logits:
            logits.append(layer(emb))
