...
class Diffsound():
    ...
    def generate_sample(self, val_path, truncation_rate, save_root, fast=False):
        ...
        acoustic_path = os.path.join('LibriTTS_1000', 'acoustic', 'acoustic_2.pth')
        acoustic_data = torch.load(acoustic_path) # get dict
        semantic_data = pd.read_csv(semantic_path, delimiter='\t') # 读取 semantic
        import time
        ...
        for index in range(len(semantic_data['item_name'])):
            ...
            semantic_tokens = torch.tensor([int(idx) for idx in semantic_str.split(' ')]).unsqueeze(0)
            ...
            acoustic_tokens = acoustic_tokens[:,:3,:] # only use 3 codebook, you can set any config.
            ...
            target_semantic_tokens = semantic_tokens[:,tmp_len:tmp_len+500] # the left 
            ...
            with torch.no_grad():
                ...
                codes = content.reshape(content.shape[0], 3, -1) # reshape to original shape
                ...
                real_out = soundstream.decode(real_acoustic_tokens.transpose(0,1))
                real_wav = real_out.detach().cpu().squeeze(0)
                save_audio(coarse_wav.cpu(), store_root+'/'+name+'.wav', 16000)
                save_audio(real_wav.cpu(), store_root+'/'+name+'_real.wav', 16000)
                #assert 1==2
...
if __name__ == '__main__':
    ...
    Diffsound.generate_sample(audio_path, truncation_rate=0.85, save_root=save_root, fast=False)
