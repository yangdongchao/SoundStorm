...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    CUDA_VISIBLE_DEVICES=4 python3 ${BIN_DIR}/hubert/dump_hubert_feature.py \
        ...
        --layer=${layer} \
        --nshard=5 \
        --rank=0 & CUDA_VISIBLE_DEVICES=4 python3 ${BIN_DIR}/hubert/dump_hubert_feature.py \
        --tsv_dir=${root_dir}/${dump_dir}/${sub_dataset_name} \
        --split=audio_files \
        --ckpt_path=${hubert_path}  \
        --feat_dir=${root_dir}/${dump_dir}/${sub_dataset_name}/semantic_feature_L${layer} \
        --layer=${layer} \
        --nshard=5 \
        --rank=1 & CUDA_VISIBLE_DEVICES=5 python3 ${BIN_DIR}/hubert/dump_hubert_feature.py \
        --tsv_dir=${root_dir}/${dump_dir}/${sub_dataset_name} \
        --split=audio_files \
        --ckpt_path=${hubert_path} \
        --feat_dir=${root_dir}/${dump_dir}/${sub_dataset_name}/semantic_feature_L${layer} \
        --layer=${layer} \
        --nshard=5 \
        --rank=2 & CUDA_VISIBLE_DEVICES=6 python3 ${BIN_DIR}/hubert/dump_hubert_feature.py \
        --tsv_dir=${root_dir}/${dump_dir}/${sub_dataset_name} \
        --split=audio_files \
        --ckpt_path=${hubert_path} \
        --feat_dir=${root_dir}/${dump_dir}/${sub_dataset_name}/semantic_feature_L${layer} \
        --layer=${layer} \
        --nshard=5 \
        --rank=3 & CUDA_VISIBLE_DEVICES=7 python3 ${BIN_DIR}/hubert/dump_hubert_feature.py \
        --tsv_dir=${root_dir}/${dump_dir}/${sub_dataset_name} \
        --split=audio_files \
        --ckpt_path=${hubert_path} \
        --feat_dir=${root_dir}/${dump_dir}/${sub_dataset_name}/semantic_feature_L${layer} \
