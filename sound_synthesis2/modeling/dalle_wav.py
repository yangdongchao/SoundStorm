...
class DALLE(nn.Module):
    ...
    def generate_content_tmp(
        ...
        def cf_predict_start(log_x_t, cond_emb, x_mask, cond_emb_mask, t):
            log_x_recon = self.transformer.predict_start(log_x_t, cond_emb, x_mask, cond_emb_mask, t)[:, :-1]
            # print('self.guidance_scale ', self.guidance_scale)
            if abs(self.guidance_scale - 1) < 1e-3:
                zero_vector = torch.zeros(x_mask.shape[0], 1, log_x_recon.shape[2]).type_as(log_x_t)- 70 
                return torch.cat((log_x_recon, zero_vector), dim=1)
            cf_cond_emb = self.transformer.empty_text_embed[:cond_emb.shape[1],:].unsqueeze(0).repeat(batch_size, 1, 1)
            cf_log_x_recon = self.transformer.predict_start(log_x_t, cf_cond_emb.type_as(cond_emb), 
                                                            x_mask, cond_emb_mask, t)[:, :-1]
            ...
            zero_vector = torch.zeros(x_mask.shape[0], 1, log_new_x_recon.shape[2]).type_as(log_x_t)- 70 
        ...
        trans_out = self.transformer.sample(batch=batch,
    ...
    def sample(
        ...
        **kwargs):
        ...
        content.update({'content_token_mask': batch['mel_token_mask']}) # 只有一个sample输入时，相当于没有mask
        content_samples = {'input_image': batch['mel_spec']} # 输入的原始mel_spec
        zshape = content['content_quant'].shape #  (B, C, H, W)
        # print('zshape ',zshape)
        if return_rec:
            content_samples['reconstruction_mel'] = self.decode_to_img(content['content_token'], zshape)
        for fr in filter_ratio:
            for cr in content_ratio:
                num_content_tokens = int((content['content_token'].shape[1] * cr)) # L*cr (L denotes the sequance length)
                ...
                if sample_type == 'debug':
                    ...
                    trans_out = self.transformer.sample_debug(condition=condition['condition'],
                    ...
                    trans_out = self.transformer.sample(condition=condition['condition'],
                                                        condition_mask=condition.get('condition_mask', None),
                                                        content_token=content_token,
                                                        filter_ratio=fr,
                                                        temperature=temperature,
