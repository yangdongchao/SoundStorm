...
decode_funcs = {"image": decode_img, "text": decode_text}
...
class ZipManager:
    ...
    def get(self, name):
        if not self._init:
            self.initialize(
                close=False
            )  # https://discuss.pytorch.org/t/dataloader-stucks/14087/3
...
class DBManager:
    ...
    def keys(self):
        ...
        if self.prefix is not None:
            _keys = [
                self.prefix + key.decode()
                for key in get_keys_from_lmdb(self.env)
            ]
        else:
    ...
    def initialize(self):
        ...
        for i, old_file in enumerate(self.files):
            ...
            self.managers[i] = Manager(
                file, self.data_type, prefix=prefix, load=False)
            print(file, " done")
