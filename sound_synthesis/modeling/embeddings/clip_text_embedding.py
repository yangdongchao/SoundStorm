...
class CLIPTextEmbedding(BaseEmbedding):
    def __init__(
        ...
        if embed_dim == 1024:
            self.embed_dim = self.text_projection.shape[
                1] * 2  # to fit 1024 dimension of image embedding
        else:
            self.embed_dim = self.text_projection.shape[
                1]  # original output, 512 dim
    ...
    def encode_text(self, text):
        text[text <
             0] = 0  # some padded text token maybe negative, so set them to 0
        ...
        if self.pick_last_embedding:
            ...
            x = x[torch.arange(x.shape[0]), text.argmax(
                dim=-1
            )] @ self.text_projection  # [batch_size, transformer.width]
    ...
    def forward(self, index, **kwargs):
        ...
        if self.additional_last_embedding is True:
            last_feature = text_feature[torch.arange(text_feature.shape[
                0]), index.argmax(dim=-1)] @ self.text_projection
            if self.keep_seq_len_dim:
