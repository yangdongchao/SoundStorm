...
def get_token_type(mask, token_shape):
    ...
    mask_unshuffle = pixel_unshuffle(mask_float,
                                     token_shape)  # B x r^2 x H/r x W/r

    scale_factor = mask_unshuffle.shape[1]
    mask_unshuffle = mask_unshuffle.sum(
        dim=1, keepdim=True)  # B x 1 x H/r x W/r
        ...
        for idx in range(content_seq_len):
            ...
            for i in range(max_pre):
                ...
                if a > half_k_w and a <= half_k_w + step_over_w:
                    ...
                    valid = True
                if valid:
                    mask[idx + condition_seq_len][idx -
                                                  i + condition_seq_len] = 1
    else:
        raise NotImplementedError(
            'attention type {} not implemented!'.format(type))

    if causal:
        causal_mask = torch.tril(
            torch.ones(content_seq_len + condition_seq_len, content_seq_len +
                       condition_seq_len))
        mask *= causal_mask
