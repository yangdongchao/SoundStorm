...
import sys
#sys.path.append('/apdcephfs/share_1316500/donchaoyang/ww/ex_code/latentSGMSE/encodec/encodec')
from encodec.encodec import quantization as qt
from encodec.encodec import modules as m
from encodec.encodec.utils import _check_checksum, _linear_overlap_add, _get_checkpoint_url
...
class EncodecModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = m.SEANetEncoder(channels=self.channels, norm=self.model_norm, causal=self.causal)
        self.decoder = m.SEANetDecoder(channels=self.channels, norm=self.model_norm, causal=self.causal)
        self.n_q = int(1000 * target_bandwidths[-1] // (math.ceil(sample_rate / self.encoder.hop_length) * 10))
    ...
    def get_tokens(self, wav):
        e = self.encoder(x)
        codes, scale = self.encode(e)
        return codes[0] # only returen the codes tensor
    def get_wav(self, codes):
        codes_ls = [codes]
        wavs_ls, sr = self.decode(codes_ls)
        return wavs_ls[0]
