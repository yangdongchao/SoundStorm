...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        if condition_emb_config is None:  # 不使用 conditional information
            ...
            self.condition_emb = instantiate(
    ...
    def p_sample(
        ...
        if t[0] > 0 and self.prior_rule > 0 and to_sample is not None:  # prior_rule: 0 for VQ-Diffusion v1, 1 for only high-quality inference, 2 for purity prior
            ...
            for i in range(log_x.shape[0]):
                n_sample = min(to_sample - sampled[i],
                               max_sample_per_step)  # 需要采样的
            ...
            sampled = [self.prior_ps] * log_x.shape[
                0]  # we change the 1024  to self.prior_ps
    ...
    def _train_loss(self,
        ...
        for index in range(t.size()[0]):
            ...
            same_rate = (tmp_x0_recon == tmp_x0_real
                         ).sum().cpu() / tmp_x0_real.shape[0]  # 
            self.diffusion_acc_list[this_t] = same_rate.item(
            ) * 0.1 + self.diffusion_acc_list[this_t] * 0.9  # 平均移动更新
                ...
                for diffusion_index in range(start_step - 1, -1, -1):  # 99,0
                    ...
                    while min(sampled) < self.n_sample[diffusion_index]:
                        log_z, sampled = self.p_sample(
                            ...
                            condition_mask, sampled, self.n_sample[
                                diffusion_index])  # log_z is log_onehot
        else:
