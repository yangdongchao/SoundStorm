import torch
from sound_synthesis2.modeling.codecs.base_codec import BaseCodec
from sound_synthesis2.modeling.modules.clip.clip import tokenize
...
class Tokenize(BaseCodec):
    def __init__(
            ...
            tokenizer_config={
                'target':
                'sound_synthesis.modeling.modules.clip.simple_tokenizer.SimpleTokenizer',
                'params': {
        ...
        if self.clip_embedding is True:
            assert condition_emb_config is not None
    ...
    def get_tokens(self, text, **kwargs):
        ...
        # assert 1==2
        if self.clip_embedding is False:
            return text_token
        else:
            if self.condition_emb.additional_last_embedding is True:
                with torch.no_grad():
                    cond_emb, last_embedding = self.condition_emb(
                        text_token['token'].cuda())
